...
class NameDetector(BaseDetector):
    ...
    def __init__(self):
        ...
        self.strong_context_patterns = [
            ...
            (r'\b(?:adım|ismim|benim\s+adım|benim\s+ismim)[\s:]+([A-ZÇĞİÖŞÜa-zçğıöşü]+)', EntityType.NAME),
            # "soyadım X", "soy adım X", "soy ismim X"
            (r'\b(?:soyadım|soyismim|soy\s*adım|soy\s*ismim)[\s:]+([A-ZÇĞİÖŞÜa-zçğıöşü]+)', EntityType.SURNAME),
            # "anne kızlık soyadı X", "annemin kızlık soyadı X"
            (r'\b(?:anne\s*kızlık\s*soyad|annemin\s*kızlık\s*soyad|kızlık\s*soyad)[ıi]?[\s:]+([A-ZÇĞİÖŞÜa-zçğıöşü]+)', EntityType.SURNAME),
            # "baba adı X", "anne adı X"
            (r'\b(?:baba\s*adı|anne\s*adı|babasının\s*adı|annesinin\s*adı)[\s:]+([A-ZÇĞİÖŞÜa-zçğıöşü]+)', EntityType.NAME),
            # "ben X", "Ben X" (cümle başı)
            (r'(?:^|\.\s+)[Bb]en\s+([A-ZÇĞİÖŞÜ][a-zçğıöşü]+)', EntityType.NAME),
            # "merhaba X", "selam X"
            (r'\b(?:merhaba|selam|günaydın|iyi\s+günler)[\s,]+([A-ZÇĞİÖŞÜ][a-zçğıöşü]+)', EntityType.NAME),
            # "müşteri X Y", "abone X Y"
            (r'\b(?:müşteri|abone|kullanıcı|üye)[\s:]+([A-ZÇĞİÖŞÜ][a-zçğıöşü]+)(?:\s+([A-ZÇĞİÖŞÜ][a-zçğıöşü]+))?', EntityType.FULL_NAME),
            # "Sayın X Y"
            ...
            # "Dr. X", "Prof. X"
            (r'\b(?:[Dd]r\.?|[Pp]rof\.?|[Dd]oç\.?|[Aa]v\.?|[Mm]üh\.?)\s+([A-ZÇĞİÖŞÜ][a-zçğıöşü]+)(?:\s+([A-ZÇĞİÖŞÜ][a-zçğıöşü]+))?', EntityType.FULL_NAME),
